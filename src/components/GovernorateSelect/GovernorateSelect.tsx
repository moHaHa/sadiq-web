...
import { TGovernorateSummary, useGovernoratesQuery } from '~/server/governorate/useGovernoratesQuery';
...
interface GovernorateSelectProps extends Omit<SelectProps, 'value' | 'onChange'> {
	init?: GovernorateOption | GovernorateOption[];
	value?: string | string[];
	onChange?: (v: string | string[], foo?: { label: string; value: string } | undefined) => void;
	valueWithName?: boolean;
}

const GovernorateSelect: FC<GovernorateSelectProps> = ({ value, onChange, valueWithName, init, ...rest }) => {
	const [searchQuery, setSearchQuery] = useState('');
	const [debouncedSearchValue, setDebouncedSearchValue] = useState('');
	const [items, setItems] = useState<TGovernorateSummary[]>([]);
	const [skip, setSkip] = useState(0);
	const { data, isLoading } = useGovernoratesQuery(
