...
import { TGeofenceSummary } from '~/server/geofence/types/item';
...
interface GeofenceSelectProps extends Omit<SelectProps, 'value' | 'onChange'> {
	zone?: string | string[];
...
const GeofenceSelect: FC<GeofenceSelectProps> = ({ zone, value, onChange, valueWithName, init, ...rest }) => {
	const [searchQuery, setSearchQuery] = useState('');
	const [debouncedSearchValue, setDebouncedSearchValue] = useState('');
	const [items, setItems] = useState<TGeofenceSummary[]>([]);
	const [skip, setSkip] = useState(0);
	const { data, isLoading } = useGeofencesQuery(
		{ name: debouncedSearchValue, limit: 20, total: true, skip: skip, zone },
		{
